...
function creerLignesFixes() {
    ...
    for (let i = 1; i <= 8; i++) {
        ...
        row.innerHTML = `
            ...
            <td><input type="number" class="prod champ-quantite" min="0" placeholder="0" value="0"></td>
            <td><input type="number" class="rebuts champ-quantite" min="0" placeholder="0" value="0"></td>
            <td>
...
function collecterDonnees() {
    ...
    if (!date || !trigramme || !equipe) {
        afficherMessage('⚠️ Veuillez remplir la date, le trigramme et l\'équipe', 'error');
        return null;
    }
    
    // Validation du trigramme (3 lettres)
    if (trigramme.length !== 3 || !/^[A-Z]{3}$/.test(trigramme)) {
        afficherMessage('⚠️ Le trigramme doit contenir exactement 3 lettres (ex: ABC)', 'error');
        return null;
    }
