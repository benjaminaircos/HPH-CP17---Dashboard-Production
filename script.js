...
const WEBHOOK_URL = "https://defaultefdc051fe1144edaa7c5efec56da13.b1.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/0c10446ba39c47b8a97cdf0d6ebf1d60/triggers/manual/paths/invoke?api-version=1";
...
// Créer les 8 lignes fixes avec heures 1h à 8h
function creerLignesFixes() {
    ...
    for (let h = 1; h <= 8; h++) {
        ...
        row.innerHTML = `
            ...
            <td><input type="number" class="prod" min="0" placeholder="0" value="0"></td>
            <td><input type="number" class="rebuts" min="0" placeholder="0" value="0"></td>
            <td>
                <select class="table-select equipement">
                    ${creerOptions(optionsEquipement)}
                </select>
            </td>
            <td><input type="number" class="equipement_duree" min="0" placeholder="0" value="0"></td>
            <td>
                <select class="table-select qualite">
                    ${creerOptions(optionsQualite)}
                </select>
            </td>
            <td><input type="number" class="qualite_duree" min="0" placeholder="0" value="0"></td>
            <td>
                <select class="table-select organisation">
                    ${creerOptions(optionsOrganisation)}
                </select>
            </td>
            <td><input type="number" class="organisation_duree" min="0" placeholder="0" value="0"></td>
            <td>
                <select class="table-select autres">
                    ${creerOptions(optionsAutres)}
                </select>
            </td>
            <td><input type="number" class="autres_duree" min="0" placeholder="0" value="0"></td>
...
function collecterDonnees() {
    ...
    if (!date || !jour || !equipe) {
        afficherMessage('⚠️ Veuillez remplir la date, le jour et l\'équipe', 'error');
        return null;
    }
    ...
    // Les lignes sont fixes de 1h à 8h
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const heure = `${i + 1}h`; // 1h, 2h, 3h, etc.
...
function initialiserDate() {
    ...
    document.getElementById('jour').value = jourActuel;
}
