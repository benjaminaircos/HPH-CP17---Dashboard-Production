...
// Graphique: Heures non justifiées par équipe
function afficherGraphiqueProductionEquipe() {
    ...
    // Compter les heures justifiées et non justifiées par équipe
    const dataParEquipe = {};
    
    // Initialiser les équipes avec 8 heures attendues
    donneesFiltrees.forEach(ligne => {
        const equipe = ligne['Équipe'] || 'Non définie';
        if (!dataParEquipe[equipe]) {
            dataParEquipe[equipe] = {
                heuresJustifiees: new Set(), // Utiliser un Set pour éviter les doublons
                totalHeures: 8 // Une équipe travaille 8 heures
            };
        }
        
        // Ajouter l'heure justifiée (extraire le chiffre de "1h", "2h", etc.)
        const heure = ligne['Heure'] || '';
        if (heure) {
            const heureNum = parseInt(heure);
            if (!isNaN(heureNum)) {
                dataParEquipe[equipe].heuresJustifiees.add(heureNum);
            }
        }
    });
    
    // Calculer les heures non justifiées pour chaque équipe
    const equipes = Object.keys(dataParEquipe).sort();
    const heuresJustifiees = equipes.map(e => dataParEquipe[e].heuresJustifiees.size);
    const heuresNonJustifiees = equipes.map(e => dataParEquipe[e].totalHeures - dataParEquipe[e].heuresJustifiees.size);
    
    // Code couleur : rouge pour heures non justifiées, vert pour heures justifiées
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        afterLabel: function(context) {
                            const equipe = context.label;
                            const heuresJ = heuresJustifiees[context.dataIndex];
                            const heuresNJ = heuresNonJustifiees[context.dataIndex];
                            
                            if (context.datasetIndex === 1 && heuresNJ > 0) {
                                return `⚠️ ${heuresNJ} heure${heuresNJ > 1 ? 's' : ''} manquante${heuresNJ > 1 ? 's' : ''}`;
                            }
                            return '';
                        }
                    }
