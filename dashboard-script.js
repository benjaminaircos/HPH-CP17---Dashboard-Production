...
function calculerKPIs() {
    ...
    let totalTempsNonJustifie = 0;
    
    // Agréger par heure pour calculer le temps non justifié
    const dataParHeure = {};
    ...
    donneesFiltrees.forEach(ligne => {
        ...
        dataParHeure[heure].prodBonne += prodBonne;
        dataParHeure[heure].rebuts += rebuts;
        ...
        const arrets = equipDuree + qualiteDuree + orgDuree + autresDuree;
        dataParHeure[heure].arrets += arrets;
        totalTempsArret += arrets;
    });
    
    // Calculer le temps non justifié pour chaque heure
    const cadence = objectifs.cadence; // pièces/minute
    
    Object.keys(dataParHeure).forEach(heure => {
        ...
        if (prodTotale < prodAttendue) {
            ...
            totalTempsNonJustifie += tempsNonJustifie;
        }
    });
    ...
    document.getElementById('kpiTempsNonJustifie').textContent = Math.round(totalTempsNonJustifie) + ' min';
...
function afficherGraphiqueTRS() {
    ...
    const trsData = heures.map(h => {
        ...
        const trs = prodTheorique > 0 ? (prodTotale / prodTheorique) * 100 : 0;
...
async function exporterPDF() {
    ...
    messageDiv.innerHTML = `
        <div style="font-size: 1.5em; margin-bottom: 15px;">📄 Génération du PDF en cours...</div>
    ...
    try {
        ...
        const originalTableDisplay = tableControls.style.display;
        const originalFilterDisplay = filterSection.style.display;
        const originalObjectifsDisplay = objectifsSection.style.display;
        ...
        // Forcer les couleurs solides pour les KPI cards avant export
        const kpiCards = document.querySelectorAll('.kpi-card');
        const originalStyles = [];
        kpiCards.forEach((card, index) => {
            originalStyles[index] = card.style.background;
            // Remplacer les dégradés par des couleurs solides
            if (index === 0 || index === 1 || index === 2) {
                card.style.background = '#667eea'; // Violet solide
            } else if (index === 3) {
                card.style.background = '#667eea'; // Violet solide
            } else if (index === 4) {
                card.style.background = '#667eea'; // Violet solide
            } else if (index === 5) {
                card.style.background = '#ff9800'; // Orange solide pour temps non justifié
            }
        });
        ...
        const opt = {
            margin: [10, 10, 10, 10],
            filename: `HPH_CP17_Dashboard_${new Date().toISOString().split('T')[0]}.pdf`,
            ...
            jsPDF: { 
                unit: 'mm', 
                format: 'a4', 
                orientation: 'landscape', // Paysage pour mieux voir les graphiques
                compress: true
            },
        ...
        // Restaurer l'affichage et les styles originaux
        headerActions.style.display = originalHeaderDisplay;
        tableControls.style.display = originalTableDisplay;
        filterSection.style.display = originalFilterDisplay;
        objectifsSection.style.display = originalObjectifsDisplay;
        
        kpiCards.forEach((card, index) => {
            card.style.background = originalStyles[index];
        });
