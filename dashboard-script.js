...
// Version IoT - Mise à jour avec données Opta
// ============================================
...
function getHorairesEquipe(dateStr) {
    ...
    const jour = date.getDay(); // 0=dim, 1=lun, 2=mar, 3=mer, 4=jeu, 5=ven, 6=sam

    if (jour === 1) { // Lundi
        return { Matin: [6, 13], Soir: [13, 21], Nuit: [21, 29] };
    } else if (jour >= 2 && jour <= 4) { // Mar/Mer/Jeu
...
function getDureeEquipe(equipe, dateStr) {
    ...
    if (!horaires[equipe]) return 480; // 8h par défaut
    const [debut, fin] = horaires[equipe];
    return (fin - debut) * 60; // en minutes
}
...
function getNbEmpreintes(reference) {
    ...
    if (ref) {
        return parseInt(ref["nombre d'empreintes"] || ref['Nb_Empreintes'] || ref['empreintes'] || 1);
    }
    return 1;
}
...
function getCadenceIoTParHeure(dateStr, equipe) {
    ...
    // Récupérer la référence en cours pour les empreintes
    const reference = document.getElementById('referenceObjectif') ? 
        document.getElementById('referenceObjectif').value.trim().toUpperCase() : '';
    const nbEmpreintes = getNbEmpreintes(reference);

    // Filtrer les événements CADENCE du jour et de l'équipe
    ...
    evenementsJour.forEach(ev => {
        ...
        // Déterminer à quelle heure d'équipe (1h à 8h) correspond cet événement
        const heureEquipe = heure - debutEquipe + 1;
        if (heureEquipe >= 1 && heureEquipe <= 8) {
            const label = `${heureEquipe}h`;
            if (!cadenceParHeure[label]) {
                cadenceParHeure[label] = [];
            }
    ...
    // Moyenner les valeurs par heure
    const resultat = {};
...
function getTempsMarcheIoTParHeure(dateStr, equipe) {
    ...
    // Filtrer les événements ON/OFF du jour
    const evenementsJour = donneesIoT.filter(ev => {
        const evDate = convertirDateExcel(ev['Date'] || '');
        let evDateISO = evDate;
        if (evDate.includes('/')) {
    ...
    // Calculer le temps de marche par heure d'équipe
    ...
    evenementsJour.forEach(ev => {
        ...
        if (etat === 'ALLUMAGE' || etat === 'ON') {
            ...
            const dureeMarche = tempsEnSecondes - dernierAllumage; // en secondes
            ...
            tempsMarcheParHeure[label] += dureeMarche / 60; // en minutes
...
function afficherGraphiqueCadenceIoT() {
    ...
    // Récupérer la date et l'équipe des données filtrées
    ...
    // Cadence nominale pour la ligne de référence
    const reference = document.getElementById('referenceObjectif') ? 
        document.getElementById('referenceObjectif').value.trim().toUpperCase() : '';
    const nbEmpreintes = getNbEmpreintes(reference);
    ...
    charts.cadenceIoT = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                y: {
                    beginAtZero: true,
                    title: { display: true, text: 'Pièces / minute' }
                },
                x: {
                    title: { display: true, text: 'Heure d\'équipe' }
                }
            },
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            return `Cadence : ${context.parsed.y.toFixed(1)} pcs/min`;
                        }
                    }
                }
            }
...
async function chargerDonnees() {
    ...
    try {
        ...
        // Nouveau format : { saisie: [...], iot: [...], empreintes: [...] }
        if (data.saisie !== undefined) {
...
function calculerKPIs() {
    ...
    // Utiliser la cadence IoT si disponible, sinon la cadence manuelle
...
function compareAvecObjectifs(totalProduction, tauxRebuts, totalTempsArret, trsGlobal) {
    ...
    donneesFiltrees.forEach(ligne => {
        if (ligne['Équipe']) equipesUniques.add(ligne['Équipe']);
    });
...
function afficherGraphiques() {
    ...
    console.log('[DEBUG vue] dates:', Array.from(datesUniques), '| équipes:', Array.from(equipesUniques), '| nb lignes:', donneesFiltrees.length);
    ...
    charts.productionHeure = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                { label: 'Objectif/Heure', data: objectifData, type: 'line', borderColor: 'rgba(255, 193, 7, 1)', borderWidth: 3, borderDash: [10, 5], pointRadius: 5, fill: false, order: 1, tension: 0 }
            ]
        },
        options: {
            responsive: true, maintainAspectRatio: true,
            scales: { y: { beginAtZero: true, title: { display: true, text: 'Quantité' } }, x: { title: { display: true, text: 'Heure' } } },
...
function afficherGraphiqueTRS() {
    ...
    const borderColor = trsData.map(val => val >= objectifs.trs ? 'rgba(75, 192, 192, 1)' : 'rgba(255, 99, 132, 1)');
    ...
    charts.trs = new Chart(ctx, {
        ...
        data: {
            labels: heures,
            datasets: [
                { label: 'TRS Réalisé (%)', data: trsData, backgroundColor, borderColor, borderWidth: 2, order: 2 },
        ...
        options: {
            ...
            scales: { y: { beginAtZero: true, max: 170, title: { display: true, text: 'TRS (%)' }, ticks: { callback: v => v + '%' } }, x: { title: { display: true, text: 'Heure' } } },
            plugins: { legend: { position: 'bottom' } }
        }
    });
}

// Graphique: Performance par heure (avec IoT si disponible)
function afficherGraphiquePerformance() {
    ...
    // Indicateur source : IoT ou déclaratif
    ...
    charts.performance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: `Performance ${sourceIoT ? '(IoT 🔵)' : '(Déclaratif)'}`,
        ...
        options: {
            responsive: true, maintainAspectRatio: true, aspectRatio: 2.2,
            scales: { y: { beginAtZero: true, max: 170, title: { display: true, text: 'Performance (%)' }, ticks: { callback: v => v + '%' } }, x: { title: { display: true, text: 'Heure' } } },
            plugins: { legend: { position: 'bottom' } }
        }
    });
}
...
function afficherGraphiqueProductionEquipe() {
    ...
    const tempsNonJustifieData = heures.map(h => {
        ...
        return Math.max(0, (ecartPieces / cadence) - data.arrets);
    });
    ...
    charts.productionEquipe = new Chart(ctx, {
        ...
        data: {
            labels: heures,
            datasets: [{ label: 'Temps Non Justifié (min)', data: tempsNonJustifieData, backgroundColor, borderColor: backgroundColor.map(c => c.replace('0.6', '1')), borderWidth: 2 }]
        },
        options: {
            responsive: true, maintainAspectRatio: true,
            scales: { y: { beginAtZero: true, title: { display: true, text: 'Minutes' } }, x: { title: { display: true, text: 'Heure' } } },
            plugins: { legend: { display: true, position: 'bottom' } }
        }
    });
}

// Graphique: Durée arrêts par catégorie
...
function determinerEquipeEnCours() {
    ...
    if (jour === 1) { debutMatin = 6; debutSoir = 13; debutNuit = 21; }
    else if (jour === 5) { debutMatin = 5; debutSoir = 13; debutNuit = 20; }
...
function appliquerFiltreEquipeParDefaut() {
    // Comportement simple et prévisible : date du jour + équipe en cours.
    // Si aucune donnée ne correspond, on affiche 0 donnée (pas de repli dans le passé).
...
function appliquerFiltres() {
    ...
    if (donneesFiltrees.length === 0) {
        alert('⚠️ Aucune donnée pour ces filtres.');
        return;
    }
    afficherDashboard();
...
async function exporterPDF() {
    ...
    window.print();
    setTimeout(() => {
        elementsToHide.forEach((el, i) => { if (el) el.style.display = originalDisplayStates[i]; });
...
function rechercherObjectifsReference() {
    ...
    // Mettre à jour le graphe cadence avec le nb d'empreintes de cette référence
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('✅ Dashboard HPH CP17 IoT initialisé');
    chargerObjectifs();
    chargerDonnees();
});
