...
async function exporterPDF() {
    ...
    try {
        ...
        // Sauvegarder la largeur originale du container
        const container = document.querySelector('.container');
        const originalWidth = container.style.width;
        const originalMaxWidth = container.style.maxWidth;
        
        // Fixer la largeur pour le rendu PDF (A4 paysage : 297mm ≈ 1120px)
        ...
        objectifsSection.style.display = '';
        container.style.width = originalWidth;
        container.style.maxWidth = originalMaxWidth;
        
        // Créer le PDF avec jsPDF
        const { jsPDF } = window.jspdf;
        const imgWidth = 297; // A4 paysage en mm
        ...
        if (imgHeight <= 210) {
            ...
            // Si le contenu est plus grand, le découper en plusieurs pages
            let heightLeft = imgHeight;
            let position = 0;
            const pageHeight = 210; // A4 paysage en mm
        ...
        // Sauvegarder le PDF
        pdf.save(`HPH_CP17_Dashboard_${new Date().toISOString().split('T')[0]}.pdf`);
        ...
        if (objectifsSection) objectifsSection.style.display = '';
        if (container) {
