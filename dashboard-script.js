...
function afficherTableau() {
    ...
    donneesFiltrees.forEach(ligne => {
        ...
        let date = ligne['Date'] || '';
        const trigramme = ligne['Trigramme'] || '';
        const equipe = ligne['Équipe'] || '';
        const heure = ligne['Heure'] || '';
...
function appliquerFiltreEquipeParDefaut() {
    ...
    donneesFiltrees = donneesCompletes.filter(ligne => {
        const equipe = ligne['Équipe'];
        console.log('🔍 Comparaison:', equipe, '===', equipeEnCours, '?', equipe === equipeEnCours);
        return equipe === equipeEnCours;
    });
    
    console.log('📊 Données filtrées sur équipe', equipeEnCours, ':', donneesFiltrees.length, 'lignes');
    
    // Si aucune donnée pour l'équipe en cours, chercher la dernière équipe avec des données
    if (donneesFiltrees.length === 0) {
        console.log('⚠️ Aucune donnée pour l\'équipe', equipeEnCours, '- Recherche de la dernière équipe avec des données');
        
        // Ordre de recherche des équipes (de la plus récente à la plus ancienne)
        const ordreEquipes = ['Nuit', 'Soir', 'Matin'];
        const indexEquipeEnCours = ordreEquipes.indexOf(equipeEnCours);
        
        // Chercher dans l'ordre : équipe précédente, puis celle d'avant, etc.
        ...
        for (let i = 1; i < ordreEquipes.length; i++) {
            ...
            if (donneesTest.length > 0) {
                ...
                donneesFiltrees = donneesTest;
                break;
            }
        }
        
        if (equipeTrouvee) {
            console.log('✅ Dernière équipe avec des données:', equipeTrouvee);
            document.getElementById('filterEquipe').value = equipeTrouvee;
        } else {
            // Si vraiment aucune équipe n'a de données, afficher toutes les données
...
function exporterCSV() {
    ...
    // Créer l'en-tête CSV
    const headers = ['Date', 'Trigramme', 'Équipe', 'Heure', 'Prod Bonne', 'Rebuts', 
