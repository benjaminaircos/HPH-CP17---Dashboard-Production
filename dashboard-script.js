...
function afficherGraphiques() {
    ...
    // Si une seule date OU pas de filtre date → graphiques HORAIRES
    // Si plusieurs dates → graphiques JOURNALIERS
    if (nbDatesUniques <= 1 && !dateDebut && !dateFin) {
        // Mode HORAIRE
    ...
    } else if (nbDatesUniques > 1 || (dateDebut && dateFin && dateDebut !== dateFin)) {
        // Mode JOURNALIER
        ...
        afficherGraphiqueEquipesJour();
    } else {
        // Par défaut : mode HORAIRE
        document.getElementById('graphiques-horaires').style.display = 'block';
        document.getElementById('graphiques-journaliers').style.display = 'none';
...
function afficherGraphiqueEquipesJour() {
    const ctx = document.getElementById('chartEquipesJour').getContext('2d');
    
    if (charts.equipesJour) {
        charts.equipesJour.destroy();
    }
    
    // Agréger par jour et par équipe
    const dataParJourEquipe = {};
    donneesFiltrees.forEach(ligne => {
        let date = convertirDateExcel(ligne['Date'] || '');
        if (date.includes('/')) {
            const parts = date.split('/');
        ...
        if (!dataParJourEquipe[date]) {
            dataParJourEquipe[date] = {};
        }
        if (!dataParJourEquipe[date][equipe]) {
            dataParJourEquipe[date][equipe] = 0;
        }
        
        dataParJourEquipe[date][equipe] += parseInt(ligne['Prod Bonne'] || ligne['Prod_x0020_Bonne'] || 0);
    });
    
    const jours = Object.keys(dataParJourEquipe).sort();
    const equipes = ['Matin', 'Soir', 'Nuit'];
    
    const datasets = equipes.map((equipe, index) => {
        const couleurs = [
            { bg: 'rgba(255, 193, 7, 0.6)', border: 'rgba(255, 193, 7, 1)' }, // Matin - Jaune
            { bg: 'rgba(54, 162, 235, 0.6)', border: 'rgba(54, 162, 235, 1)' }, // Soir - Bleu
            { bg: 'rgba(153, 102, 255, 0.6)', border: 'rgba(153, 102, 255, 1)' }  // Nuit - Violet
        ];
        
        return {
            label: equipe,
            data: jours.map(j => dataParJourEquipe[j][equipe] || 0),
            backgroundColor: couleurs[index].bg,
            borderColor: couleurs[index].border,
            borderWidth: 2
        };
    });
    
    charts.equipesJour = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: jours.map(d => new Date(d).toLocaleDateString('fr-FR')),
            datasets: datasets
