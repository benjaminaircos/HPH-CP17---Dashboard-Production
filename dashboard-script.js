...
function chargerObjectifs() {
    ...
    if (stored) {
        ...
        document.getElementById('objectifProdJour').value = objectifs.prodJour;
        document.getElementById('objectifTauxRebuts').value = objectifs.tauxRebuts;
        document.getElementById('objectifTempsArret').value = objectifs.tempsArret;
    }
}

// Enregistrer les objectifs
function appliquerObjectifs() {
    objectifs.prodJour = parseInt(document.getElementById('objectifProdJour').value) || 5000;
    objectifs.tauxRebuts = parseFloat(document.getElementById('objectifTauxRebuts').value) || 5;
    objectifs.tempsArret = parseInt(document.getElementById('objectifTempsArret').value) || 120;
...
function compareAvecObjectifs(totalProduction, tauxRebuts, totalTempsArret) {
    ...
    kpiObjectifProd.textContent = `Objectif: ${objectifs.prodJour.toLocaleString()}`;
    
    if (totalProduction >= objectifs.prodJour) {
        ...
        const pourcentage = ((totalProduction / objectifs.prodJour) * 100).toFixed(0);
    ...
    // Temps d'arrêt
    const kpiCardArret = document.getElementById('kpiCardTempsArret');
    const kpiObjectifArret = document.getElementById('kpiObjectifArret');
    const kpiStatusArret = document.getElementById('kpiStatusArret');
    
    kpiObjectifArret.textContent = `Objectif max: ${objectifs.tempsArret} min`;
    
    if (totalTempsArret <= objectifs.tempsArret) {
        kpiStatusArret.textContent = '✅ Objectif respecté';
        kpiStatusArret.className = 'kpi-status atteint';
        kpiCardArret.className = 'kpi-card success';
    } else {
        kpiStatusArret.textContent = '❌ Objectif dépassé';
        kpiStatusArret.className = 'kpi-status non-atteint';
        kpiCardArret.className = 'kpi-card danger';
    }
}
