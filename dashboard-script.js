...
function afficherFiltresActifs() {
    ...
    if (dateDebut && dateFin) {
        const dateD = new Date(dateDebut).toLocaleDateString('fr-FR');
        const dateF = new Date(dateFin).toLocaleDateString('fr-FR');
        texte += `Date : ${dateD} - ${dateF}`;
    } else if (dateDebut) {
        const dateD = new Date(dateDebut).toLocaleDateString('fr-FR');
        texte += `Date : À partir du ${dateD}`;
    } else if (dateFin) {
        const dateF = new Date(dateFin).toLocaleDateString('fr-FR');
        texte += `Date : Jusqu'au ${dateF}`;
    } else {
        texte += 'Toutes les dates';
    }
    ...
    if (equipe) {
        ...
        texte += 'Toutes les équipes';
    }
    
    document.getElementById('filtresTexte').textContent = texte;
}
...
function calculerKPIs() {
    ...
    // Compter le nombre d'heures dans les données filtrées
    ...
    // Calculer le TRS Global : (Prod Totale / (Cadence × 60 × Nb heures)) × 100
