...
async function chargerDonnees() {
    ...
    try {
        ...
        // Afficher toutes les données sans filtre automatique
        donneesFiltrees = [...donneesCompletes];
        calculerKPIs();
        afficherGraphiques();
        afficherTableau();
...
function initialiserFiltresDates() {
    ...
    // Par défaut, on affiche la date du jour
...
async function rechercherObjectifsReference() {
    ...
    // DÉSACTIVÉ TEMPORAIREMENT - Problème d'authentification Power Platform
    // La recherche automatique ne fonctionne pas à cause des restrictions de sécurité
    
    const messageElem = document.getElementById('messageEnregistrement');
    messageElem.textContent = `ℹ️ Saisie manuelle pour ${reference}`;
    ...
    console.log(`ℹ️ Recherche automatique désactivée - Saisie manuelle requise`);
    
    /* CODE DÉSACTIVÉ - À réactiver quand l'authentification sera résolue
    try {
