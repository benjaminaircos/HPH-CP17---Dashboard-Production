...
function calculerMarcheIoTParHeure(dateISO, equipe) {
    ...
    for (let i = 0; i < evts.length - 1; i++) {
        if (evts[i].etat === 'ALLUMAGE' && evts[i+1].etat === 'EXTINCTION') {
            const durSec = evts[i+1].ts - evts[i].ts;
            // Attribuer à l'heure de l'équipe (1h, 2h...) où démarre le segment de marche
            const hEquipe = evts[i].heure - debH + 1;
            const lbl = `${hEquipe}h`;
            if (!marcheParHeure[lbl]) marcheParHeure[lbl] = 0;
            marcheParHeure[lbl] += durSec / 60;
        }
    }
...
function chartTempsMarche(canvasId, heures, data) {
    ...
    makeOrUpdate('tempsMarche', canvasId, {
        type: 'bar',
        data: { labels: heures, datasets: [
            { label:'Temps de marche (min)', data, backgroundColor:colors, borderColor:colors.map(c=>c==='transparent'?c:c.replace('bb','ff')), borderWidth:1, borderRadius:3, order:2, spanGaps:true },
            { label:'Heure pleine (60 min)', data:heures.map(()=>60), type:'line', borderColor:CHART_COLORS.warn, borderDash:[8,4], borderWidth:2, pointRadius:0, fill:false, order:1 }
        ]},
        options: { ...baseChartOpts('Durée (min)', 65) }
    });
}
