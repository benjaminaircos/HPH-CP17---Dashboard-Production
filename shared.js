...
// ── Durée réelle de l'équipe (pondérée au temps écoulé) ─────
function getDureeEquipeReelle(dateISO, equipe) {
    const hor = getHorairesEquipe(dateISO);
    const [debH, finH] = hor[equipe] || [5, 13];
    const today = new Date().toISOString().split('T')[0];

    if (dateISO === today) {
        // Jour en cours : heure actuelle - début équipe
        const maintenant = new Date();
        const heureActuelle = maintenant.getHours() + maintenant.getMinutes() / 60;
        // Plafonner à la fin de l'équipe
        const heureEffective = Math.min(heureActuelle, finH);
        // Si on n'est pas encore dans l'équipe
        if (heureActuelle < debH) return (finH - debH) * 60;
        return Math.max(1, Math.round((heureEffective - debH) * 60));
    }

    // Date passée : dernier événement IoT de l'équipe
    ...
    if (evts.length > 0) {
        // Trouver le dernier événement de l'équipe
        const dernier = evts.reduce((max, ev) => {
            const ts = parseInt(ev['Heure']||0) * 3600 + parseInt(ev['Minute']||0) * 60 + parseInt(ev['Seconde']||0);
            const tsMax = parseInt(max['Heure']||0) * 3600 + parseInt(max['Minute']||0) * 60 + parseInt(max['Seconde']||0);
            return ts > tsMax ? ev : max;
        });
        const hFin = parseInt(dernier['Heure']||0) + parseInt(dernier['Minute']||0) / 60;
        return Math.max(1, Math.round((hFin - debH) * 60));
    }

    // Fallback : durée fixe de l'équipe
    return (finH - debH) * 60;
}
