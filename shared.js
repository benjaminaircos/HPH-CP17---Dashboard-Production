...
// ── Durée réelle basée sur les heures saisies ───────────────
// TRS Global calculé uniquement sur les heures effectivement produites
// Objectif TRS reste basé sur la durée totale fixe de l'équipe
function getDureeDepuisSaisies(lignes) {
    if (!lignes || !lignes.length) return 60; // fallback 1h
    // Extraire les heures uniques saisies (1h, 2h, 3h...)
    const heures = new Set(lignes.map(l => parseInt(l['Heure']||0)));
    // Nombre d'heures distinctes × 60 min
    return Math.max(1, heures.size * 60);
}
...
function normaliserDate(val) {
    ...
    // Cas déjà ISO (YYYY-MM-DD) — vérifier EN PREMIER
    if (s.match(/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/)) return s;
    // Cas string avec / (DD/MM/YYYY)
    if (s.includes('/')) {
        const p = s.split('/');
...
// ── Temps de MARCHE réel IoT par heure (inverse de calculerArretIoTParHeure) ──
function calculerMarcheIoTParHeure(dateISO, equipe) {
    if (!donneesIoT.length || !dateISO) return {};
    const hor = getHorairesEquipe(dateISO);
    const [debH, finH] = hor[equipe] || [5, 13];
    ...
    // Calculer marche par heure calendaire : Σ(ALLUMAGE → EXTINCTION),
    // en découpant chaque segment aux frontières des heures pleines
    // pour ne pas attribuer toute la durée à une seule heure quand
    // la machine tourne sans interruption sur plusieurs heures.
    const marcheParHeure = {};
    for (let i = 0; i < evts.length - 1; i++) {
        if (evts[i].etat === 'ALLUMAGE' && evts[i+1].etat === 'EXTINCTION') {
            let debutSeg = evts[i].ts;
            const finSeg = evts[i+1].ts;
            while (debutSeg < finSeg) {
                const heureCal = Math.floor(debutSeg / 3600);
                const finHeureCal = (heureCal + 1) * 3600;
                const finPortion = Math.min(finSeg, finHeureCal);
                const dureeSec = finPortion - debutSeg;
                const hEquipe = heureCal - debH + 1;
                const lbl = `${hEquipe}h`;
                if (!marcheParHeure[lbl]) marcheParHeure[lbl] = 0;
                marcheParHeure[lbl] += dureeSec / 60;
                debutSeg = finPortion;
            }
        }
    }
    Object.keys(marcheParHeure).forEach(k => {
        marcheParHeure[k] = Math.round(marcheParHeure[k]);
    });
    return marcheParHeure;
}
...
// Toutes les dates sont maintenant en texte ISO yyyy-MM-dd
// On délègue à normaliserDate() qui gère déjà ce format
function normaliserDateIoT(val) {
    return normaliserDate(val);
}
...
function chartPerformance(canvasId, heures, perfData) {
    const colors = perfData.map(v => v === null ? 'transparent' : v >= 90 ? CHART_COLORS.ok+'bb' : v >= 75 ? CHART_COLORS.warn+'bb' : CHART_COLORS.danger+'bb');
    makeOrUpdate('performance', canvasId, {
        ...
        data: { labels: heures, datasets: [
            ...
            { label:'Objectif (90%)', data:heures.map(()=>90), type:'line', borderColor:CHART_COLORS.warn, borderDash:[8,4], borderWidth:2, pointRadius:0, fill:false, order:1 }
        ]},
        options: { ...baseChartOpts('Performance (%)', 170) }
    });
}

// ── Graphique Temps de fonctionnement (marche) par heure ──────
function chartTempsMarche(canvasId, heures, data) {
    const colors = data.map(v => v === null ? 'transparent' : v >= 50 ? CHART_COLORS.ok+'bb' : v >= 30 ? CHART_COLORS.warn+'bb' : CHART_COLORS.danger+'bb');
    makeOrUpdate('tempsMarche', canvasId, {
        type: 'bar',
        data: { labels: heures, datasets: [
            { label:'Temps de marche (min)', data, backgroundColor:colors, borderColor:colors.map(c=>c==='transparent'?c:c.replace('bb','ff')), borderWidth:1, borderRadius:3, spanGaps:true }
        ]},
        options: { ...baseChartOpts('Durée (min)', 65), plugins:{ legend:{display:false} } }
    });
}
