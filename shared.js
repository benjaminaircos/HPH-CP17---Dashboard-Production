...
// ── Cache empreintes (localStorage, durée 24h) ───────────────
const CACHE_EMP_KEY = 'hph_cp17_empreintes';
const CACHE_EMP_TTL = 24 * 60 * 60 * 1000; // 24h en ms

function lireCacheEmpreintes() {
    try {
        const raw = localStorage.getItem(CACHE_EMP_KEY);
        if (!raw) return null;
        const { ts, data } = JSON.parse(raw);
        if (Date.now() - ts > CACHE_EMP_TTL) { localStorage.removeItem(CACHE_EMP_KEY); return null; }
        return data;
    } catch(e) { return null; }
}

function ecrireCacheEmpreintes(data) {
    try { localStorage.setItem(CACHE_EMP_KEY, JSON.stringify({ ts: Date.now(), data })); } catch(e) {}
}
...
async function chargerDonnees(urlSaisie, urlIoT, urlEmpreintes, dateDebut, dateFin) {
    try {
        // Ajouter dateDebut/dateFin aux URLs saisie et IoT si fournis
        let urlSaisieFiltered = urlSaisie;
        let urlIoTFiltered    = urlIoT;
        if (dateDebut && dateFin) {
            urlSaisieFiltered += `&dateDebut=${dateDebut}&dateFin=${dateFin}`;
            if (urlIoT) urlIoTFiltered = urlIoT + `&dateDebut=${dateDebut}&dateFin=${dateFin}`;
        }

        // Vérifier cache empreintes
        const cacheEmp = urlEmpreintes ? lireCacheEmpreintes() : null;
        if (cacheEmp) {
            donneesEmpreintes = cacheEmp;
            console.log(`📦 Empreintes depuis cache (${cacheEmp.length})`);
        }

        const [rSaisie, rIoT, rEmp] = await Promise.all([
            fetch(urlSaisieFiltered, { method: 'GET' }),
            urlIoTFiltered ? fetch(urlIoTFiltered, { method: 'GET' }) : Promise.resolve(null),
            (urlEmpreintes && !cacheEmp) ? fetch(urlEmpreintes, { method: 'GET' }) : Promise.resolve(null)
        ]);
        ...
        if (rIoT && rIoT.ok) {
            const d = await rIoT.json();
            donneesIoT = d.value || (Array.isArray(d) ? d : []);
        }
        if (rEmp && rEmp.ok) {
            const d = await rEmp.json();
            donneesEmpreintes = d.value || (Array.isArray(d) ? d : []);
            ecrireCacheEmpreintes(donneesEmpreintes);
        }
...
function chartCadenceIoT(canvasId, dateISO, equipe, intervalMin) {
    ...
    for (let i = 0; i < nb; i++) {
        ...
        const evts = donneesIoT.filter(ev => {
            ...
            const evDate = normaliserDate(ev['Date'] || ev['date'] || '');
